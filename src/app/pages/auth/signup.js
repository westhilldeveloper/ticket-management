...
export default function Signup() {
  ...
  const { sendOtp } = useAuth(); // <-- new function, does NOT create user
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Step 1: Send OTP only – no user data is saved yet
      const result = await sendOtp(formData.email); // assumes sendOtp exists
      if (result.success) {
        setShowOTP(true);
      } else {
  ...
  // After OTP is sent, render OTPVerification and pass the full form data
  if (showOTP) {
    return <OTPVerification email={formData.email} formData={formData} />;
  }
    ...
    <div className="min-h-screen flex items-center justify-center bg-gray-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl w-full space-y-8">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit} noValidate>
          ...
          {/* GDPR Consent - spans both columns */}
